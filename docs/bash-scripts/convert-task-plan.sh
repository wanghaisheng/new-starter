...
# 输出文件
RESULTS_FILE="docs/tasks/task-conversion-results.txt"

# 清空或创建结果文件
echo "任务计划转换结果报告" > $RESULTS_FILE
echo "=========================" >> $RESULTS_FILE
...
check_environment() {
  ...
  if [ "$current" -lt "$required" ]; then
    echo "❌ 环境要求不满足: $3" >> $RESULTS_FILE
    return 1
  fi
  echo "✅ 环境要求满足: $3" >> $RESULTS_FILE
  return 0
}
EOL
...
while IFS= read -r line; do
    ...
    # 检查是否是子任务
        ...
        elif [[ "$subtask" =~ ^更新配置[[:space:]]+(.+)$ ]]; then
            config_file="${BASH_REMATCH[1]}"
            echo "check_file_exists \"$config_file\" || echo \"⚠️ 配置文件 $config_file 不存在\"" >> "$SCRIPT_FILE"
        elif [[ "$subtask" =~ ^运行测试[[:space:]]+(.+)$ ]]; then
            test_path="${BASH_REMATCH[1]}"
            echo "echo \"运行测试: $test_path...\" >> \$RESULTS_FILE" >> "$SCRIPT_FILE"
            echo "bun test \"$test_path\"" >> "$SCRIPT_FILE"
        elif [[ "$subtask" =~ ^构建项目$ ]]; then
            echo "echo \"构建项目...\" >> \$RESULTS_FILE" >> "$SCRIPT_FILE"
            echo "bun run build" >> "$SCRIPT_FILE"
        else
