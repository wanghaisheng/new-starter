...
export class CloudflareD1Client extends BaseClient implements IDatabaseClient {
  ...
  async findAll<T extends BaseEntity>(tableName: string, filter?: Record<string, any>): Promise<T[]> {
    ...
    try {
      // 使用 Drizzle ORM 查询
      if (this.drizzleDB && tableName in drizzleSchema && this.canUseOrmForFilter(filter)) {
        const table = drizzleSchema[tableName as keyof typeof drizzleSchema];
        ...
        if (filter && Object.keys(filter).length > 0) {
          const conditions = Object.entries(filter).map(([key, value]) => {
            return eq(table[key as keyof typeof table] as any, value);
          });
          
          // 使用类型断言解决Drizzle类型问题
          query = this.drizzleDB.select().from(table).where(and(...conditions)) as any;
        }
    ...
    try {
      ...
      if (this.drizzleDB && tableName in drizzleSchema) {
        ...
        if (options.where) {
          ...
          if (whereCondition) {
            // 使用类型断言解决Drizzle类型问题
            query = this.drizzleDB.select().from(table).where(whereCondition) as any;
            countQuery = this.drizzleDB.select({ count: sql`count(*)` }).from(table).where(whereCondition) as any;
          }
        }
    ...
    try {
      ...
      if (this.drizzleDB && tableName in drizzleSchema && this.canUseOrmForFilter(filter)) {
        ...
        if (filter && Object.keys(filter).length > 0) {
          ...
          // 使用类型断言解决Drizzle类型问题
          query = this.drizzleDB.select({ count: sql`count(*)` }).from(table).where(and(...conditions)) as any;
        }
  ...
  // 检查是否可以使用ORM进行过滤
  private canUseOrmForFilter(filter?: Record<string, any>): boolean {
    if (!filter || Object.keys(filter).length === 0) {
      return true;
    }
    ...
    if ('field' in whereOption && 'operator' in whereOption && 'value' in whereOption) {
      ...
      switch (operator) {
        ...
        case '$in':
          ...
          return { whereClause: `${field} = ?`, whereParams: params };
        case '$contains':
          params[params.length - 1] = `%${value}%`; // 替换为LIKE模式
          return { 
