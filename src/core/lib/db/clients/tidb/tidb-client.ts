import { Kysely, MysqlDialect, ReferenceExpression, CompiledQuery, Compilable, sql } from 'kysely';
 ...
 * TiDB数据库客户端
 */
export class TiDBClient implements IDatabaseClient {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log('TiDBClient initialized successfully');
    } catch (error) {
      console.error('Error initializing TiDBClient:', error);
      throw error;
    }
  }
  ...
  private checkInitialized(): void {
    if (!this._isInitialized) {
      throw new Error('TiDBClient not initialized');
    }
  }
} 