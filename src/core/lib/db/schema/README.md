...
### 风险说明
- **禁止在表名中使用连字符（`-`）**：如 `member-growth-tasks`，会导致 Drizzle ORM 生成的 migration SQL 在 SQLite 等数据库下报错（如 `near "-": syntax error`）。
- **表名中包含特殊字符（如 `-`、空格等）时，SQL 层需加双引号**，否则部分数据库不兼容。
- **表名与文件名、类型名无直接映射关系**，表名以 schema 定义对象的 `name` 字段为准。

### 最佳实践
- **统一采用下划线风格（snake_case）定义表名**，如 `member_growth_tasks`、`global_configs`，避免一切特殊字符。
- **表名应为全小写，单词间用下划线分隔，建议用复数形式**，如 `users`、`photos`、`member_growth_tasks`。
- **如需自动生成表名，务必加一层校验/转换逻辑**，将所有 `-`、空格等自动转为下划线。
- **历史表名如含 `-`，需批量修正为下划线风格，并同步修正相关 SQL/接口等引用。**

### 典型错误案例
- 错误：`name: 'member-growth-tasks'`
- 正确：`name: 'member_growth_tasks'`

### 相关问题与处理
- Drizzle ORM/SQLite migration 报错时，优先检查表名、索引名、列名是否含有特殊字符。
- 统一在表结构 schema 定义阶段规范命名，避免后续迁移和兼容性问题。
