...
### **9. 业务服务插件集成实践（phone 相关服务）**

本项目在 `src/core/services-update/business/phone/` 目录下的服务（如 `bluetooth-service.ts`、`geolocation-service.ts` 等），均支持通过 Capacitor 插件实现跨平台能力：

- **Web/PWA 环境**：优先使用标准 Web API（如 `navigator.bluetooth`）。
- **原生 App 环境**：通过 Capacitor 插件（如 `@capacitor/bluetooth-le`、`@capacitor/geolocation` 等）实现同一接口的原生能力。
- **服务接口设计**：所有业务服务均定义统一接口（如 `IBluetoothService`），并根据运行环境自动选择实现。
- **插件集成示例**：
  - 蓝牙服务：
    - Web 端用 `navigator.bluetooth` 实现扫描、连接
    - App 端用 `@capacitor/bluetooth-le` 实现完整蓝牙功能
  - 定位服务：
    - Web 端用 `navigator.geolocation`
    - App 端用 `@capacitor/geolocation`

> 参考：`src/core/services-update/business/phone/bluetooth-service.ts` 及相关实现。

**集成建议：**
- 尽量保持接口一致，便于前端/业务代码无感切换
- 插件能力不足时可用 JS Polyfill 或降级方案
- 插件需在 Capacitor 官方或社区维护、兼容主流平台
...
### **11. 如果你们在 **App Store 上架的 iOS 应用**中同时集成了 **RevenueCat（苹果内购 IAP）** 和 **Stripe（或其他第三方支付）**，**确实存在被苹果下架的风险**，但具体是否违规取决于支付的使用场景。以下是关键规则和应对策略：
