...
2. **Service Adapter 层**
   ...
   - 支持多种实现方式：
     ...
     - Better 适配器：用于生产环境
     - FakeIndexedDB 适配器：用于 mock 阶段，支持纯内存 mock 和离线/在线混合场景
   - 典型文件：`src/core/services/data/adapters/{sqlite|firebase|mock}-data-service.ts`

3. **Factory/Registry 层**
   - 负责创建服务实例
   - 根据配置选择适当的适配器
   - 实现单例模式确保全局唯一实例
   - 典型文件：`src/core/services/data/factory/data-service-factory.ts`
...
所有业务服务注册表（Registry）需统一实现如下方法签名和行为，确保多实例支持、自动降级和调用一致性：

### getService 方法

```typescript
getService(environment: string, name: string = 'default'): ServiceType | undefined
```
- **参数**：
  - `environment`：服务环境标识（如 'mock'、'remote'、'hybrid' 等）
  - `name`：实例名（默认 'default'，支持多实例场景）
- **返回**：已注册的服务实例或 undefined
- **说明**：所有 Registry 均应以 `environment:name` 作为实例缓存 key，保证多环境多实例兼容。

### getDefaultService 方法

```typescript
getDefaultService(_dataService?: unknown): ServiceType
```
- **参数**：
  - `_dataService`：部分服务可能需要的数据服务实例，未用时可省略
- **返回**：优先级顺序的默认服务实例（如 remote > hybrid > mock），兜底自动创建 mock 实例
- **说明**：所有业务 hooks 均通过此方法获取默认服务实例，禁止直接通过 Factory 创建。

### registerAllAdapters 方法

```typescript
static registerAllAdapters(): void
```
- **说明**：注册所有内置适配器，仅传 type，不传 name，确保与工厂方法签名一致。

### 统一规范要求
- Registry 内部实例缓存 key 统一为 `environment:name`
- getService 必须支持 name 默认值
- getDefaultService 必须按 remote > hybrid > mock 顺序兜底
- 所有页面、hooks、服务均通过 Registry 获取实例，禁止 Factory 直连
- 适配器注册方法只传 type，不传 name
...
1. **创建适配器**
   ```typescript
   // src/core/services/business/auth/adapters/nextauth/nextauth-auth-service.ts
...
2. **注册提供者**
   ```typescript
   // src/core/services/business/auth/registry/auth-registry.ts
...
3. **更新配置类型**
   ```typescript
   // src/core/services/business/auth/types/config.ts
...
1. **创建适配器**
   ```typescript
   // src/core/services/business/auth/adapters/clerk/clerk-auth-service.ts
...
2. **同步策略配置**
   ```typescript
   // src/core/services/data/types/config.ts
...
3. **同步管理器实现**
   ```typescript
   // src/core/services/business/sync/sync-manager.ts
