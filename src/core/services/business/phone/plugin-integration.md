...
本项目在 `src/core/services/business/phone/` 目录下的服务（如 `bluetooth-service.ts`、`geolocation-service.ts` 等），均支持通过 Capacitor 插件实现跨平台能力：
...
> 参考：`src/core/services/business/phone/bluetooth-service.ts` 及相关实现。
