...
// [整改建议]
// 1. 保证所有泛型、接口、方法签名与实际 registry/factory 体系一致
// 2. getService/getProvider 统一用 provider/providerFactory 方案，getService 方法仅用于兼容老代码
// 3. ServiceConfig、IService、IServiceFactory、IServiceRegistry 类型建议仅供底层基础设施/扩展用，业务 hooks/页面不直接依赖
// 4. ServiceError、ServiceStatus、ServiceEventListener 保持导出，供全局错误处理与事件监听使用